...
namespace HotelManagement.DAO
{
	...
	internal class HoaDonPhongDAO
	{
		...
		public DataTable getAllHoaDonPhong()
		{
			...
			string sql = "SELECT HDPhong.MaHDP, MaNV, MaKH, NgayDat, CTHDP.NgayNhan, Lp.TenLP, CTHDP.NgayTra, CTHDP.MaP, CTHDP.TrangThai, format(P.GiaMacDinh * (CASE WHEN DATEDIFF(DAY, CTHDP.NgayNhan, CTHDP.NgayTra) = 0 THEN 1 ELSE DATEDIFF(DAY, CTHDP.NgayNhan, CTHDP.NgayTra) END),'N0') AS \"Thành Tiền\" FROM HoaDonPhong HDPhong JOIN CTHDPhong CTHDP ON HDPhong.MaHDP = CTHDP.MaHDP  JOIN Phong P ON CTHDP.MaP = P.MaP JOIN LoaiPhong Lp ON Lp.MaLP = P.MaLP;";
